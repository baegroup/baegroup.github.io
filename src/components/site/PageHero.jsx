...
export function PageHero({ title, description, tags = [], children, showDescription = false }) {
  ...
  return (
    <section className={`pb-1 pt-1 md:pb-2 ${revealClassName}`} ref={ref} style={revealStyle}>
      <div className={children ? 'md:flex md:items-end md:justify-between md:gap-8' : ''}>
        ...
        {children ? <div className="mt-5 border-b border-slate-200 md:mt-0 md:shrink-0">{children}</div> : null}
      </div>
    </section>
  );
}
